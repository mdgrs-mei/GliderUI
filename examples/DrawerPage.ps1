...
function CreateDrawerMenuItem($Title, $IconGeometry, $NavigationPage) {
    ...
    $button.AddClick({
            param($argumentList)
            $page, $navigationPage = $argumentList

            # Always replace the root page as we are using the NavigationPage only to get the transition effect this time.
            # The navigation stack is not used.
            $navigationPage.ReplaceAsync($page) | Out-Null
