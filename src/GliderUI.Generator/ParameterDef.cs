...
internal class ParameterDef
{
    ...
    public static string GetParametersSignatureExpression(List<ParameterDef> parameters, List<TypeDef>? genericTypeParametersOverride, bool isExtensionMethod)
    {
        if (parameters.Count == 0)
            return "";

        StringBuilder builder = new();
        string split = isExtensionMethod ? "this " : "";
        foreach (var parameter in parameters)
        {
            _ = builder.Append($"{split}{parameter.GetSignatureExpression(genericTypeParametersOverride)}");
            split = ", ";
        }
        return builder.ToString();
    }
