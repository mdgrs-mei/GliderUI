...
Since the API of GliderUI follows the Avalonia UI's API, you can read the Avalonia UI documentation to see what should be available. The documentation of the `Button` class is [here](https://docs.avaloniaui.net/docs/reference/controls/buttons/button) for example.

You can also refer to the [examples](./examples/Examples.md) folder for script samples.

## How It Works

*GliderUI* launches a server process `GliderUI.Server` that provides all the UI functionalities. The GliderUI module communicates with the server through IPC (Inter-Process Communication) to create UI elements and handle events. No Avalonia UI's dlls are loaded in PowerShell.
...
All Avalonia UI controls and types become accessible by adding "GliderUI" as a prefix to their namespaces.
